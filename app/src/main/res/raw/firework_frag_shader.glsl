...
#define NUM_SUB_SPARKLES 2.

#define SUB_SPARKLE_CHANCE .2
...
#define MIN_CAM_DISTANCE 4.
#define MAX_CAM_DISTANCE 4.
...
void initCamera(vec2 uv, vec3 position, vec3 lookAt, float zoom) {
...
vec4 map( in vec3 p) {
    ...
    float d = sdCapsule(p+0.0*normal, vec3(0., -10., 0.), vec3(0., 10., 0.), .1);

    return vec4(d, 0.0, transition, 2.);
}
...
vec4 mainImage( in vec2 fragCoord )
{
    ...
    float t = iGlobalTime*NORMAL_MOTION_SPEED;

    float turn = pi*2.+iGlobalTime*.1;
    float s = sin(turn);
    float c = cos(turn);
    mat3 rot = mat3(	  c,  0., s,
                          0., 1., 0.,
    ...
    CAMERA_DISTANCE = mix(MIN_CAM_DISTANCE, MAX_CAM_DISTANCE, sin(iGlobalTime*.0765)*.5+.5);
    vec3 pos = vec3(0., 0.4, -CAMERA_DISTANCE)*rot;

    initCamera(uv, pos, vec3(0., 2.3, 0.), 1.);
    float dist;
    render(cam.ray.o, cam.ray.d, dist);

    vec3 col = background(cam.ray);
    col += sparkles(cam.ray, uv, t, NORMAL_MOTION_SPEED, dist);
    col = Rainbow(col);
